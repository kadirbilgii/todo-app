...
export class TeamsService {
  ...
  async findTeams(req: any) {
    const userId = await this.user.findOne({ where: { username: req.user.username } }).then((user) => {
        if (!user) {
    ...
    const teamIds =  await this.teamUsersModel.findAll({ where: { userId: req.user.sub } }).then((teamUsers) => {
        if (!teamUsers) {
  ...
  async findUsers(teamId: string, req: any) {
    ...
    await this.teamUsersModel.findOne({ where: {userId: req.user.sub} }).then((teamUser) => {
        if (!teamUser) {
            throw new ForbiddenException('User is not a member of this team');
        }
    });

    const userIds = await this.teamUsersModel.findAll({ where: { teamId } }).then((teamUsers) => {
        if (!teamUsers) {
            throw new NotFoundException('Team user not found');
        }
        return teamUsers.map((teamUser) => {
  ...
  async addUser(addMemberDto: AddMemberDto) {
    ...
    const userId = await this.user.findOne({ where: { username } }).then((user) => {
        if (!user) {
            throw new NotFoundException('User not found');
        }
        return user.id;
     ...
     await this.teamUsersModel.findOne({
        where: { userId, teamId },
     }).then((teamUser) => {
        if (teamUser) {
